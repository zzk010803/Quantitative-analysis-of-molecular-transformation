...
description:
	...
	This program will search the mass output for the dehydration during the anaerobic digestion.
...
while(my $line = <$SITE2>){
	...
	# search for the decarboxylation line by line
	for(my $i=0;$i<scalar(@site1);$i++){
		# same sample with different number of oxygen atoms
		if($site1[$i] eq $fields[1] && $O1[$i] != $fields[5]){
			# dehydration: the reduction of oxygen and hydrogen atoms followed a stoichiometric ratio of 1:2
			if($C1[$i] == $fields[3] && $P1[$i] == $fields[8] && $S1[$i] == $fields[7] && $N1[$i] == $fields[6] && ($H1[$i]-$fields[4])/($O1[$i]-$fields[5]) == 2){
				my $o_diff = $O1[$i]-$fields[5]; # number of reduced oxygen atoms
				
				# write to the output file
				print $OUT "$sample1[$i]	$fields[0]	$mass1[$i]	$fields[2]	$El_comp_1[$i]	$fields[10]	$Class_1[$i]	$fields[11]	$MolForm_1[$i]	$fields[9]	$o_diff\n";
			}
		}
	}
}
