...
description:
	...
	This program will search the mass output for the demethylation during the anaerobic digestion.
...
while(my $line = <$SITE2>){
	...
	# search for the demethylation line by line
	for(my $i=0;$i<scalar(@site1);$i++){
		# same sample with different number of hydrogen atoms
		if($site1[$i] eq $fields[1] && $H1[$i] != $fields[4]){
		
			# demethylation: the reduction of carbon and hydrogen atoms followed a stoichiometric ratio of 1:2
			if($O1[$i] == $fields[5] && $P1[$i] == $fields[8] && $S1[$i] == $fields[7] && $N1[$i] == $fields[6] && ($C1[$i]-$fields[3])/($H1[$i]-$fields[4]) == 0.5){
