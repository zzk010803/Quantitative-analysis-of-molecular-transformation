...
description:
	...
	This program will search the mass output for the deamination during the anaerobic digestion.
...
while(my $line = <$SITE2>){
	...
	# search for the deamination line by line
	for(my $i=0;$i<scalar(@site1);$i++){
		# same sample with different number of nitrogen atoms
		if($site1[$i] eq $fields[1] && $H1[$i] != $fields[4] && $O1[$i] != $fields[5] && $N1[$i] != $fields[6]){
			# deamination: the reduction of nitrogen, hydrogen and oxygen atoms followed a stoichiometric ratio of 1:3:1
			if($C1[$i] == $fields[3] && $P1[$i] == $fields[8] && $S1[$i] == $fields[7] && ($H1[$i]-$fields[4])/($N1[$i]-$fields[6]) == 3 && ($N1[$i]-$fields[6]) == ($fields[5]-$O1[$i])){
				my $n_diff = $N1[$i]-$fields[6]; # number of reduced nitrogen atoms
				
				# write to the output file
				print $OUT "$sample1[$i]	$fields[0]	$mass1[$i]	$fields[2]	$El_comp_1[$i]	$fields[10]	$Class_1[$i]	$fields[11]	$MolForm_1[$i]	$fields[9]	$n_diff\n";
			}
		}
	}
}
